...
async def build_aggregation_prompt(
    ...
    """
    Build targeted aggregation prompt based on available data
    """
    ...
    if has_primary_attrs:
        primary_list = "\n".join([f"  {i+1}. {attr}" for i, attr in enumerate(primary_attributes)])
...
async def get_taxonomy_attribute_hints(
    ...
    if category:
        ...
        attr_stmt = select(Attribute.display_name).join(
            CategoryAttribute,
            Attribute.id == CategoryAttribute.attribute_id
        ).where(
            CategoryAttribute.category_id == category.id,
            CategoryAttribute.is_primary == True  
        ).order_by(
            CategoryAttribute.display_order
        )
        
        attr_result = await db.execute(attr_stmt)
        attributes = [row[0] for row in attr_result.all()]
        
        if attributes:
            logger.info(f"Found {len(attributes)} primary attributes for '{taxonomy}'")
            return attributes
    ...
    for i in range(len(taxonomy_parts), 0, -1):
        partial_path = ' > '.join(taxonomy_parts[:i])
        
        stmt = select(Category).where(Category.full_path == partial_path)
        result = await db.execute(stmt)
        category = result.scalars().first()
        ...
        if category:
            ...
            if attributes:
                logger.info(f"Partial match: '{partial_path}' -> {len(attributes)} attributes")
                return attributes
    ...
    if taxonomy_parts:
        leaf_category_name = taxonomy_parts[-1]
        
        stmt = select(Category).where(
            Category.name.ilike(f"%{leaf_category_name}%"),
        ...
        if category:
            ...
            if attributes:
                logger.info(f"Leaf match: '{leaf_category_name}' -> {len(attributes)} attributes")
                return attributes
    ...
    if category:
        ...
        if attributes:
            logger.info(f"Fallback: Using top {len(attributes)} attributes")
            return attributes
    
    
    logger.warning(f"No attributes found for taxonomy: '{taxonomy}'")
    return []
