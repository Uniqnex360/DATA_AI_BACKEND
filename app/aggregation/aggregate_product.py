...
def is_likely_product_image(img_url: str, alt: str = '') -> bool:
    ...
    if alt and any(k in alt_lower for k in product_keywords):
        score += 1
    # If score >= 2, consider it a good candidate
...
#     # 3. First <img> with product‑related alt or class
#     product_img = soup.find('img', src=re.compile(r'product|main|hero|full', re.I))
#     if product_img and product_img.get('src'):
#         return urljoin(base_url, product_img['src'])

#     # 4. Any image with reasonable size (assume larger than 100px)
...
async def extract_fallback_image(html: str, base_url: str) -> Optional[str]:
    ...
    if meta and meta.get('content'):
        ...
        candidates.append((url, 100))  # high base score
    ...
    for url, score in candidates:
        if score > 20:   # threshold to avoid junk
...
async def aggregate_product(
    ...
    try:
        ...
        # urls = await search_service.search_product(mpn, brand)
        urls = await search_service.get_urls(query)
                ...
                # Call LLM with structured output
                    ...
                    if pdf_text and len(pdf_text.strip()) > 100:
                        ...
                        async with _llm_semaphore:
                            ...
                            prompt=prompt_config['prompt'],
                            response_model="ExtractionResponse",
                            estimated_tokens=4000  # PDFs often have more text
                        )
                            await asyncio.sleep(0.5)
                        if extraction_result and extraction_result.product_detected:
                            attr_dicts = []
        ...
        #     source_index_map[idx] = source
        #     for attr in source ['attributes']:
        #         attr_name=attr.get('name','unknown')
        #         combined_for_cleaning[f"source_{idx}_{attr_name}"] = attr.get('value', '')
        #     from app.aggregation.prompts.cleaning_prompts import build_cleaning_prompt
        #     cleaning_config = build_cleaning_prompt(source['attributes'])
        #     async with _llm_semaphore:
        ...
        #         )
        #     for idx,source in source_index_map.items():
        #         cleaned_attrs = [
        #             attr for attr in cleaning_result.cleaned_attributes
        #             if isinstance(attr, dict) and str(attr.get('name', '')).startswith(f"source_{idx}_")
        #         ]  
        #         for attr in cleaned_attrs:
        #             attr['name'] = attr['name'].replace(f"source_{idx}_", "")
                            
        #     cleaned_sources.append({
        #         **source,
        #         'attributes': cleaning_result.cleaned_attributes,
        ...
        # Combine all cleaned attributes
        all_cleaned_attrs = []
        ...
        # ========================================
        # STAGE 5: VALIDATION (if in backfill/validate mode)
        # ========================================
        ...
        if "back filling" in use_case or "validation" in use_case:
            ...
            # Get web attributes (from aggregation)
            web_attrs = {}
        ...
        # (Keep existing standardization logic)
        ...
        return {
            ...
            'golden_record': {
                ...
                'short_description': enrichment_result.short_description,
...
def apply_unification(sources: List[Dict], groups: List) -> List[Dict]:
    ...
    """
    # Create mapping: old_name -> canonical_name
    mapping = {}
    for group in groups:
