...
#                 used.add(name_j)
#                 logger.info(f"  {name_i} ≈ {name_j} (sim={similarity:.2f})")
...
_embedding_model = None
_db_attr_cache: Dict[UUID, Dict] = {}  # category_id -> {"names": [...], "embeddings": np.ndarray, "meta": [...]}

_CONTAINER_TERMS = {
    'carton', 'pack', 'pallet', 'box', 'case', 'each', 'unit', 'piece', 'bag', 'sleeve'
}

AUTO_MERGE_THRESHOLD = 0.92   # near-certain: case/spelling/whitespace variants -> merge, skip LLM
LLM_CONFIRM_THRESHOLD = 0.55  # ambiguous zone: ask LLM to confirm/reject
LOCAL_CLUSTER_THRESHOLD = 0.85  # fallback clustering among attrs with no DB match
...
async def get_embedding_model():
    """Cache the model at module level. Previous version checked a global
    from app.main that was never set, so it reloaded the model every call."""
...
async def get_category_canonical_attributes(
    ...
    """Pull existing canonical attribute names for THIS category only, plus
    a few sample values already stored for each, so the LLM can tell apart
    same-name-different-concept cases (e.g. Net Weight vs Gross Weight).

    Scoping by category_id matters: 'Diameter' means something different in
    Grinders vs Hoses, so a global lookup would cause cross-category
    false merges.
    """
    ...
    rows = result.all()
    logger.info(f"[CategoryCanonicals] db_load category_id={category_id} rows={len(rows)}")
    logger.info(f"[CategoryCanonicals] db_names_units={[(name, unit) for _, name, unit in rows][:120]}")
    meta = []
    ...
    logger.info(f"[CategoryCanonicals] meta_built category_id={category_id} meta_count={len(meta)}")
    logger.info(f"[CategoryCanonicals] meta_names_sample={[(m['name'], m.get('unit')) for m in meta[:60]]}")
    model = await get_embedding_model()
    names = [m["name"] for m in meta]
    embeddings = await asyncio.to_thread(model.encode, names) if names else np.array([])

    _db_attr_cache[cache_key] = {"names": names, "embeddings": embeddings, "meta": meta}
    return meta


def invalidate_category_cache(category_id: UUID) -> None:
    """Call this if canonical attributes for a category change mid-session."""
...
async def _llm_confirm_batch(
    ...
    """One LLM call for all ambiguous raw names in this product, not one
    call per name. ambiguous = [{"raw_name": ..., "raw_value": ..., "raw_unit": ...,
    "candidates": [{"name": ..., "unit": ..., "sample_values": [...]}]}]
    Returns {raw_name: matched_canonical_or_None}
    """
    if not ambiguous:
        return {}
...
async def cluster_attributes_by_meaning(
    ...
    # --- Stage A: match against DB-scoped canonical attributes ---
    if category_id and db:
        ...
        if db_meta:
            ...
            for i, name in enumerate(attr_names):
                sims = [1 - cosine(raw_embeddings[i], db_embeddings[j]) for j in range(len(db_names))]
    ...
    f"sim={best_sim:.3f} top3={[(m['name'], round(s,3)) for m, s in ranked[:3]]}"
                ...
                if best_sim >= AUTO_MERGE_THRESHOLD:
                    logger.info(f"  Auto-merge (sim={best_sim:.2f}): '{name}' -> '{best_meta['name']}'")
                    canonical_map[name] = best_meta['name']
                elif best_sim >= LLM_CONFIRM_THRESHOLD:
                    top_candidates = [m for m, s in ranked[:3] if s >= LLM_CONFIRM_THRESHOLD]
                    ...
                    logger.info(f"[DBMatch] llm_confirm_batch_size={len(llm_batch)}")
                else:
    ...
    # --- Stage B: local clustering among names with no DB match ---
    if len(unresolved) >= 2:
        logger.info(f"Local clustering {len(unresolved)} attrs with no DB match")
        ...
        for i, name_i in enumerate(unresolved):
            ...
            for j, name_j in enumerate(unresolved):
                ...
                if _should_skip_container_clustering(name_i, name_j):
                    continue
                similarity = 1 - cosine(unresolved_embeddings[i], unresolved_embeddings[j])
                if similarity >= threshold:
                    cluster.append(name_j)
                    used.add(name_j)
            clusters.append(cluster)
        ...
        for cluster in clusters:
            if len(cluster) == 1:
                ...
                continue
            if canonical_names:
                matched_existing = next((n for n in cluster if n in canonical_names), None)
                if matched_existing:
                    for alias in cluster:
