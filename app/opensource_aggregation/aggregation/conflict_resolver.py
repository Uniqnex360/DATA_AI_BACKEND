...
class ConflictResolver:
    """
    Resolve attribute conflicts using confidence scoring

    Strategy:
    1. If all sources agree → use that value
    2. If conflict → use highest confidence source
    3. If tie → use most frequent value
    """
    ...
    def resolve(
        ...
        """Build the golden record from unified attributes"""
    ...
    def _pick_best_value(
        ...
        """
        Pick the best value for an attribute

        Returns (best_value, has_conflict)
        """
        ...
        # All agree (after normalization)
        if len(normalized) == 1:
            best = values[0]
            return best.value, False

        # Conflict — use scoring
        has_conflict = True
        ...
        for norm_value, attrs in normalized.items():
            # Combined score = sum of confidence + frequency bonus
            confidence_sum = sum(a.confidence for a in attrs)
            frequency_bonus = len(attrs) * 0.1  # Bonus for agreement
        ...
        # Return original (non-normalized) value
    ...
    def _calculate_overall_confidence(
        ...
        """Calculate overall confidence of the golden record"""
